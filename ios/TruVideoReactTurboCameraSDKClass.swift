...
@objc final public class TruVideoReactCameraSdkClass: NSObject {
  ...
  @objc public func initCameraScreen(jsonData: String,onResolve: @escaping(_ cameraResult :String) -> Void,onReject: @escaping(_ cameraResult :String) -> Void){
        print(jsonData)
        guard let data = jsonData.data(using: .utf8) else {
            print("Invalid JSON string")
          onReject("Invalid JSON string")
          return
        }
        do {
          ...
          onReject("Error parsing JSON: \(error.localizedDescription)")
        }
    }
